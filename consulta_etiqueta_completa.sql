-- Consulta completa por etiqueta: carcaça id, produção, regra id, qualidade id e status da regra
-- Esta consulta retorna todas as informações relacionadas a uma etiqueta específica
...
LEFT JOIN modelo m ON m.id = c.modelo_id
LEFT JOIN medida md ON md.id = c.medida_id
WHERE c.numero_etiqueta = ? -- Substitua ? pelo número da etiqueta
